...
export default function NumRowsInput(props: { defaultValue: number[]; tableID?: string }) {
  const { t } = useTranslation(['frequent']);
  const { defaultValue, tableID } = props;
    ...
    selectedValue === -1 ? (
      ...
      <FormControl>
        <Select
          ...
          size="small"
          variant="outlined"
          {...(tableID && { labelId: tableID, 'aria-labelledby': tableID })}
        >
          {options.map(option => {
