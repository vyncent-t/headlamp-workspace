...
const RESOURCE_TYPES = ['Pod'] as const;
type ResourceType = (typeof RESOURCE_TYPES)[number];

const RESOURCE_CLASSES: Record<ResourceType, { getBaseObject: () => any }> = {
  Pod,
};
...
function CreateActivityContent(props: { onClose: () => void }) {
  ...
  function handleResourceChange(resource: ResourceType | '') {
    setSelectedResource(resource);
    if (resource && resource in RESOURCE_CLASSES) {
      const baseObject = RESOURCE_CLASSES[resource as ResourceType].getBaseObject();
      setItem(baseObject);
  ...
  function renderFormContent() {
    return (
      ...
      >
        <FormControl sx={{ alignSelf: 'start', width: '80%', maxWidth: 480, marginLeft: 20 }}>
        ...
        {selectedResource === 'Pod' && (
          <CreatePodForm resource={formResource} onChange={handleFormChange} />
        )}
      </Box>
    );
  }
