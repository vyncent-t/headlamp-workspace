...
interface LogsButtonProps {
  ...
  location?: 'split-left' | 'split-right' | 'full' | 'window';
}
...
function LogsButtonContent({ item, location }: LogsButtonProps) {
  ...
  const isSplit = location === 'split-left' || location === 'split-right';
  const sideSnapStyles = {
    xs: 'column',
    lg: isSplit ? 'column' : 'row',
  };
  const sideSnapMargin = isSplit ? 0 : 2;
...
function LogsButtonContentWrapper({ item }: LogsButtonProps) {
  const [activity] = useActivity();
  return <LogsButtonContent location={activity.location} item={item} />;
}
...
export function LogsButton({ item }: LogsButtonProps) {
  ...
  const onClick = () => {
    ...
    Activity.launch({
      ...
      cluster: item.cluster,
      location: 'full',
      content: <LogsButtonContentWrapper item={item} />,
    });
  };
