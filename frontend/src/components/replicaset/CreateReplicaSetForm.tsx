...
const nextMatchLabelRowId = () => `rs-match-label-${++matchLabelRowIdCounter}`;
...
export default function CreateReplicaSetForm(props: CreateReplicaSetFormProps) {
  ...
  React.useEffect(() => {
    ...
    const next = _.cloneDeep(normalizedResource) as ReplicaSetDraft;
    let changed = false;
  ...
  function handleMatchLabelsChange(nextMatch: Record<string, string>) {
    ...
    const nextResource = _.cloneDeep(normalizedResource) as ReplicaSetDraft;
